  ...
  server: {
    handlers: {
      GET: handler("calendar.read", async ({ request, ctx }) => {
        ...
        return json(request, { version: found?.id ?? null, data: found?.days ?? [] }, { requestId: ctx.requestId });
      }),
      ...
      POST: handler("calendar.write", async ({ request, ctx }) => {
        ...
        if (!body.period || !Array.isArray(body.days)) throw new ApiError("period and days are required.");
        const created = await saveCalendar(ctx.factoryId, body.period, body.days, ctx.user.id);
        await audit(ctx, "CALENDAR_CHANGE", "CALENDAR_VERSION", created.id, { new: { days: body.days.length } });
